...
def project_detail_view(request, pk):
    ...
    if current_user.is_authenticated:
        ...
        is_favorited = current_user.favorites.filter(pk=project_instance.pk).exists() 
    view_context = {
...
def project_create_view(request):
    ...
    if request.method != 'POST':
        ...
        return render(request, 'projects/create-project.html', {'form': empty_form, 'is_edit': False})
    ...
    return render(request, 'projects/create-project.html', {'form': filled_form, 'is_edit': False})
...
def toggle_favorite_view(request, pk):
    ...
    is_already_favorited = user_profile.favorites.filter(id=project_obj.id).exists()
...
def close_project_view(request, pk):
    ...
    if target_project.owner != current_user:
        return JsonResponse({'status': 'error', 'message': 'У вас нет прав на это действие'}, status=403)
...
def toggle_participate_view(request, pk):
    ...
    is_participating = current_user.participated_projects.filter(id=target_project.id).exists()
    if is_participating:
